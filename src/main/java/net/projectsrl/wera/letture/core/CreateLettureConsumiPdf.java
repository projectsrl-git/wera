...
public class CreateLettureConsumiPdf extends CreateALIMODPdf_base {

    public CreateLettureConsumiPdf(String outDirectory, Map<String, Object> map) throws Exception {

        super(outDirectory, map);
        
        
        String idCondominio = map.get("ID_CONDOMINIO").toString();
        String[] dati = new String[setDimension(idCondominio)+1];
        Integer numero=1;
        ...
        try {
            ...
            ac.logContext(this.getClass().getName(),
                    "Errore nella ricerca dell'ultimo progressivo del dataset " + "DSLettureConsumiCondominiDettaglioStampa");
            try {
				throw ac;
			} catch (AppCrash e) {
        ...
        setFixedNumberOfPages(setDimension(idCondominio)+1);
    ...
    private int setDimension(String idCondominio) {
    	 DataSet_itf dataSet = null;
         DataSetFactory dsFactory = DataSetFactory.getInstance();
         Integer dimensione=0;

         try {
             dataSet = dsFactory.makeDataSet("", "DSLettureConsumiStampaNumeroRighe");
