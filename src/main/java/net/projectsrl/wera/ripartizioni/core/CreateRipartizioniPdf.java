...
public class CreateRipartizioniPdf extends CreateALIMODPdf_base {

    public CreateRipartizioniPdf(String outDirectory, Map<String, Object> map) throws Exception {

        super(outDirectory, map);
                
        
        String idModulo = map.get("ID_MODULO").toString();
        Integer pagineDettagli = setDimension(idModulo);
        Integer pagineFinali=setDimensionUltimePagine(idModulo);
        //if (pagineFinali<2){
        	pagineFinali=pagineFinali+1;
        //}
        String[] dati = new String[pagineDettagli+pagineFinali];
        Integer numero=1;
        ...
        dati[0] = "RIPARTIZIONI-TESTATA"+alias+".pdf";
        Integer ultimePagine=1;
        Integer inizioTotali=0;
        
                
        try {
            dataSet = dsFactory.makeDataSet("", "DSRipartizioniDettaglioStampa");
            ...
            while (dataSet.hasMoreElements()) {
            	...
            	dati[numero]="RIPARTIZIONI-DETTAGLI-"+numero+".pdf";
                numero= numero+1;
            }
        	...
        	while (dataSet.hasMoreElements()) {
            	...
            	if (numero%26==0){
            		...
            		dati[inizioTotali]="RIPARTIZIONI-ULTIMA-PAGINA-"+ultimePagine+".pdf";
            		numero= numero+1;
            ...
            ac.logContext(this.getClass().getName(),
                    "Errore nella ricerca dell'ultimo progressivo del dataset " + "DSRipartizioniDettaglioStampa");
            try {
				throw ac;
			} catch (AppCrash e) {
            ...
            if (dataSet != null) {
                try {
                    ...
                    ac.logContext(this.getClass().getName(), "Errore nella close del dataset " + "DSRipartizioniDettaglioStampa");
                }
            }
        }
        ...
        //setFixedNumberOfPages(setDimension(idModulo)+setDimensionUltimePagine(idModulo)+1);
        setFixedNumberOfPages(pagineDettagli+pagineFinali);
	...
	private void creazionePagineConNomiCampi(Integer numero, String alias) throws DocumentException {
		try {
        String src=getOutDirectory().replace(WebAppConstants_itf.OUTPUT_PATH, WebAppConstants_itf.TEMPLATES_PATH)+"RIPARTIZIONI-DETTAGLI-1"+alias+".pdf";
        String dest=getOutDirectory().replace(WebAppConstants_itf.OUTPUT_PATH, WebAppConstants_itf.TEMPLATES_PATH)+"RIPARTIZIONI-DETTAGLI-"+numero+".pdf";
        ...
        form.renameField("MANUTENZIONE_STRAORDINARIA_1", "MANUTENZIONE_STRAORDINARIA_"+numeroNuovo);
    ...
    private int setDimension(String idModulo) {
         ...
         try {
             dataSet = dsFactory.makeDataSet("", "DSRipartizioniDettaglioStampaNumeroRighe");
             ...
             ac.logContext(this.getClass().getName(),"Errore nella ricerca dell'ultimo progressivo del dataset " + "DSRipartizioniDettaglioStampaNumeroRighe");
         } finally {
             // chiude il dataset per il conteggio degli elementi trovati
             if (dataSet != null) {
                 try {
                     ...
                     AppCrash ac = new AppCrash(t);
                     ac.logContext(this.getClass().getName(), "Errore nella close del dataset " + "DSRipartizioniDettaglioStampaNumeroRighe");
                 }
             }
         }
		return dimensione;
	...
	private void creazioneUltimePagine(Integer numero, String alias) throws DocumentException {
		try {
        String src=getOutDirectory().replace(WebAppConstants_itf.OUTPUT_PATH, WebAppConstants_itf.TEMPLATES_PATH)+"RIPARTIZIONI-ULTIMA-PAGINA"+alias+".pdf";
        String dest=getOutDirectory().replace(WebAppConstants_itf.OUTPUT_PATH, WebAppConstants_itf.TEMPLATES_PATH)+"RIPARTIZIONI-ULTIMA-PAGINA-"+numero+".pdf";
        Integer numeroNuovo=numero%26;
        PdfReader reader;
		reader = new PdfReader(src);
		PdfStamper stamper = new PdfStamper(reader, new FileOutputStream(dest));
        AcroFields form = stamper.getAcroFields();
        ...
        if (numeroNuovo>1){
        	for (int i=1;i<27;i++){
                ...
                form.renameField("MILLESIMI_"+i, "MILLESIMI_"+(i+(26*(numeroNuovo-1))));
                ...
                form.renameField("METANO_"+i, "METANO_"+(i+(26*(numeroNuovo-1))));
                form.renameField("CONDUZIONE_"+i, "CONDUZIONE_"+(i+(26*(numeroNuovo-1))));
                form.renameField("FORZA_MOTRICE_"+i, "FORZA_MOTRICE_"+(i+(26*(numeroNuovo-1))));
                form.renameField("COSTI_LETTURE_"+i, "COSTI_LETTURE_"+(i+(26*(numeroNuovo-1))));
                form.renameField("TOTALE_CON_LETTURE_"+i, "TOTALE_CON_LETTURE_"+(i+(26*(numeroNuovo-1))));
                form.renameField("MANUTENZIONE_STRAORDINARIA_"+i, "MANUTENZIONE_STRAORDINARIA_"+(i+(26*(numeroNuovo-1))));
            }
        }
