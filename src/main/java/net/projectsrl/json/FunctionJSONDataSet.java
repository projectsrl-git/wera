...
public class FunctionJSONDataSet extends FunctionProjectWebApp_base {

	public FunctionJSONDataSet(ApplicationServices_itf applServices, String functionID, String functionName) {
		...
		try {
			...
			while (dataSet.hasMoreElements()) {
				...
				for (int i = 0; i < outputColumns.length; i++) {
					...
					//String escapedFieldValue = StringEscapeUtils.escapeJavaScript(filedValue);
					
					if (filedValueObject instanceof Number) {
						jsonTail.append("\"" + outputColumns[i] + "\":" + escapedFieldValue);
