...
public abstract class FunctionInserimentoALIMOD_base<D extends AliModDAO_base> extends FunctionAjaxForm_base<D> {

    public FunctionInserimentoALIMOD_base(ApplicationServices_itf applServices, String functionID,
            String functionName) {
    ...
    protected void sendResponseJSON(SsbServletResponse res, boolean result, String title, String message, D formDao) {
        ...
        try {
            ...
            Integer idAzienda = 0;
            if (formDao.getAttribute(AliModDAO_base.ID_AZIENDA)!=null){
            	idAzienda = Integer.parseInt(formDao.getAttribute(AliModDAO_base.ID_AZIENDA).toString());
            }
    ...
    protected void update(SsbServletRequest req, D formDao, UserSecurityInfo userInfo) throws AppCrash {
        ...
        String stato = req.getField(AliModDAO_base.STATO);

        formDao.setAttribute(AliModDAO_base.ID_MODULO, idModulo);

        formDao.retrieve();

        String nrModulo = (String) formDao.getAttribute(AliModDAO_base.NR_MODULO);
        if (stato == null || stato.equals("")) {
            stato = (String) formDao.getAttribute(AliModDAO_base.STATO);
        }

        formDao.setAttributesFromRequest(req);

        formDao.setAttribute(AliModDAO_base.NR_MODULO, nrModulo);
        formDao.setAttribute(AliModDAO_base.STATO, stato);
