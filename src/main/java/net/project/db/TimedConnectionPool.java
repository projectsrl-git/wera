 ...
 *
 */
public class TimedConnectionPool extends ConnectionPool_base implements ConnectionPool_itf, Runnable {

    private ArrayList _available;
    private HashSet   _busy = new HashSet();
     ...
     */
    public TimedConnectionPool(String name) throws AppCrash {
        ...
        try {
            ...
            ErrDetector.GetInstance().invariant(
                    (_connectionNum != 0) && (_dbPassword != null) && (_dbUser != null) && (_connectionURL != null)
                            && (_connectionTimeout != 0) && (_JDBCDriver != null));

            _available = new ArrayList(_connectionNum);
        ...
        while (true) {
            ...
            try {
                ...
                synchronized (this) {
                    ...
                    ArrayList newAvailable = new ArrayList(_connectionNum);
                    ...
                    Logger.GetInstance().log0(
                            "TimedConnectionPool available " + _available.size() + " busy " + _busy.size());
                }
            } catch (Throwable err) {
    ...
    public DBConnection_itf getAConnection() throws AppCrash {
        ...
        try {
            ...
            synchronized (this) {
                // Se non vi sono connessioni attive ne creo una
                if (_available.size() == 0) {

                    ErrDetector.GetInstance().invariant(_busy.size() < _connectionMax,
                            "Raggiunto limite connessione DB in uso");

                    DBConnection conn = DBFactory.makeTimedDBConnection(_configName, _connectionURL, _dbUser,
                            _dbPassword);
                    ConnectionContainer container = new ConnectionContainer(conn);
                    _available.add(container);
                }

                // Prendo la prima connessione attiva dalla testa della lista
                ConnectionContainer container = (ConnectionContainer) _available.remove(0);
                DBConnection conn = container.getConnection();
     ...
     *
     * @see net.project.db.ConnectionPool_base#freeNotify(net.project.db.DBConnection_itf)
     */
    @Override
